...
async function checkIngredients() {
  try {
    ...
    const shared = await Ingredient.find({ cartId: null }).select('name isActive').lean();
    ...
    if (shared.length > 0) {
      ...
      console.log("   Super admin must create ingredients (they will automatically have cartId: null)");
    }
